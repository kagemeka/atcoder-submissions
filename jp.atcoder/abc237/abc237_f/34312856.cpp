...
mint dp[1 << 10][11][11][11];
#define rep(i, n) for(int i = 0; i < n; i++)
auto main() -> int {
  int n, m;
  cin >> n >> m;
  // m++;
  // vector<vector<vector<mint>>> dp(m, vector<vector<mint>>(m,
  // vector<mint>(m))); for(int i = 0; i < n; i++) {
  //   vector<vector<vector<mint>>> ndp(
  //     m, vector<vector<mint>>(m, vector<mint>(m))
  //   );
  // }
  dp[0][m][m][m] = 1;
  rep(i, n) rep(a1, m + 1) rep(a2, m + 1) rep(a3, m + 1) {
    mint& v = dp[i][a1][a2][a3];
    if(v == 0) continue;
    rep(x, m) {
      if(x <= a1) dp[i + 1][x][a2][a3] += v;
      else if(x <= a2) dp[i + 1][a1][x][a3] += v;
      else if(x <= a3) dp[i + 1][a1][a2][x] += v;
    }
  }
  mint s = 0;
  rep(a1, m) rep(a2, m) rep(a3, m) s += dp[n][a1][a2][a3];
  cout << s << endl;
}
