...
// GenerateFacIFac .
func GenerateFacIFac(n, p int) ([]int, []int) {
...
// Init .
func (pn *PrimeNumbers) Init(n int) {
	pn.IsPrime, pn.Values = SieveOfEratosthenes(n)
}
...
// Factorize .
func (pn *PrimeNumbers) Factorize(n int) map[int]int {
...
// FactorizeFactorial .
func (pn *PrimeNumbers) FactorizeFactorial(n int) map[int]int {
	res := make(map[int]int)
	for i := 1; i < n+1; i++ {
		for p, c := range pn.Factorize(i) {res[p] += c}
	}
	return res
}
...
func (cn *Combinatorics) Init(n, mod int) {
	...
	cn.mod = mod
	cn.fac, cn.ifac = GenerateFacIFac(n, mod)
}
...
func Permutations(n, r int) [][]int {
	var Perm func(a []int, r, i int) (res [][]int) 
	Perm = func(a []int, r, i int) (res [][]int) {
		n := len(a)
		if r>n || i>r {return}
		if i==r {return append(res, a[:r])}
		for j := i; j < n; j++ {
...
func main() {
	...
	// AtCoder.Contests["ABC057"]["D"]()
	AtCoder.Contests["ABC181"]["D"]()
