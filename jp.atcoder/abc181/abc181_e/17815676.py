...
class AtCoder:
  ...
  class ABC181:
    ...
    def d():
      s = sys.stdin.readline().rstrip()
      for i in range(1000):
        if i %8 != 0 and not '0' in str(i): print(i)
    ...
    def e():
      ...
      if n == 1:
        print(min(abs(x-h[0]) for x in w))
        return
      r = [0] * n
      ...
      for i in range(n):
        j = bi_l(w, h[i])
        if j == 0: d[i] += w[0]-h[i]
        elif j == m: d[i] += h[i]-w[-1]
        else: d[i] += min(h[i]-w[j-1], w[j]-h[i])

      res = inf
      for i in range(n):
        if i & 1:
          res = min(res, d[i] + l[min(i+2, n-1)] + r[max(i-2, 0)])
        else:
          # res = min(res, d[i]+)
          res = min(res, d[i]+l[min(i+1, n-1)]+r[max(i-1, 0)])
      print(res)
