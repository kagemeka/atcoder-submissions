...
auto main() -> int {
  ...
  h = y_all.size();
  w = x_all.size();
  vector<vector<int>> x_list(h), y_list(w);

  vector<int> y_all2(y_all.begin(), y_all.end()),
    x_all2(x_all.begin(), x_all.end());

  for(int i = 0; i < n + 2; i++) {
    y[i] = lower_bound(y_all2.begin(), y_all2.end(), y[i]) - y_all2.begin();
    x[i] = lower_bound(x_all2.begin(), x_all2.end(), x[i]) - x_all2.begin();
  ...
  // cout << h << endl;
  // for(int i = 0; i < n + 2; i++) { cout << y[i] << " \n"[i == n + 1]; }
  for(int i = 0; i < h; i++) sort(x_list[i].begin(), x_list[i].end());
  for(int i = 0; i < w; i++) sort(y_list[i].begin(), y_list[i].end());
