...
class Combinatorics:
  ...
  class CombinationsMod:
    ...
    def make_nchoose_table(self, n):
      ...
      r = len(self.__fac)+1
      if using_numpy:
        self.__n_choose = n_choose = np.arange(n+1, n-r, -1); n_choose[0] = 1
        n_choose[1:] = self.__cumprod(n_choose[1:])*self.__ifac[1:r+1]%p
      else:
        self.__n_choose = n_choose = [None]*(r+1); n_choose[0] = 1
        for i in range(r): n_choose[i+1] = n_choose[i]*(n-i)%p
        for i in range(1,r+1): n_choose[i] = n_choose[i]*self.__ifac[i]%p
      return self.__n_choose
