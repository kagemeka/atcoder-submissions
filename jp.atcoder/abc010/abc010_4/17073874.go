...
func (g *Graph) FloydWarshall() map[interface{}]map[interface{}]int {
	d := make(map[interface{}]map[interface{}]int)
	for u := range g.nodes {
		d[u] = make(map[interface{}]int)
		for v := range g.nodes {d[u][v] = 1001001001001001 }
	}
	for v := range g.nodes {d[v][v] = 0}
	for u, tmp := range g.edges {
		for v, e := range tmp {d[u][v] = e.weight}
	}
	for w := range g.nodes {
		for u := range g.nodes {
			for v := range g.nodes {
