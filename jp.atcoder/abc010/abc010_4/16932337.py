...
class GeometryTopology:
  class GraphTheory:
    class Flow:
      @staticmethod
      def dinic_max_flow(graph, s, t): # s: source, t: sink
        def bfs():
          level = defaultdict(int); level[s] = 0
          q = deque([s])
          while q:
            u = q.popleft()
            for v, cap in graph[u].items():
              if cap == 0 or v in level: continue
              level[v] = level[u]+1
              q.append(v)
          return level
  ...
  class ABC010:
    ...
    def d_3():
      ...
      for a, b in zip(*[map(int, sys.stdin.read().split())]*2):
        graph[a][b] = 1
        graph[b][a] = 1
      max_flow = GeometryTopology.GraphTheory.Flow.dinic_max_flow
      print(max_flow(graph, 0, n))
