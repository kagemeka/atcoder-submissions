...
// var MaximumFlow = Module{
func MaximumFlowDinic(graph map[int]map[int]int, s, t int) int {
	bfs := func() map[int]int {
		level := make(map[int]int); level[s] = 0
		q := []int{s}
		for len(q) > 0 {
			u := q[0]; q = q[1:]
			for v, cap := range graph[u] {
				if cap == 0 {continue}
				if _, visited := level[v]; visited {continue}
				level[v] = level[u]+1
				q = append(q, v) 
			}
		}
	...
	var flowToSink func(u, flowIn int, level map[int]int) int 
	flowToSink = func(u, flowIn int, level map[int]int) int {
		if u == t {return flowIn}
		flow := 0
		for v, cap := range graph[u] {
			if cap == 0 {continue}
			if level[v] <= level[u] { continue}
			f := flowToSink(v, MinInt(flowIn, cap), level)
			if f == 0 { continue }
			// fmt.Println(f)
			graph[u][v] -= f 
			graph[v][u] += f
	...
	for {
		level := bfs()
		if _, visited := level[t]; !visited {return flow}
		flow += flowToSink(s, math.MaxInt64, level)
	}
	...
	contests: map[string]contest{
		...
		"ABC009": contest{
			...
			"D": func() {
				...
				var bitwise_mat_pow func(a [][]int, n int) [][]int 
				bitwise_mat_pow = func(a [][]int, n int) [][]int {
					res := make([][]int, k)
					for i := 0; i < k; i++ {
						res[i] = make([]int, k)
						res[i][i] = mask
					...
					if n&1 == 1 {res = bitwise_dot(res, a)}
					return res
				}
				...
				fmt.Println(bitwise_dot(bitwise_mat_pow(d, m-k), b)[0][0])
		...
		"ABC010": contest{
			...
			"D": func() {
				...
				fmt.Println(MaximumFlowDinic(graph, 0, n))
			},
		},
