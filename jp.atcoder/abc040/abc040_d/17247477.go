		...
		"ABC040": contest{
			...
			"D": func() {
				...
				for _, tmp := range q {
					...
					if y&1==1 {res[j] = uf.s[uf.FindRoot(i)]} else {uf.Unite(i, j)}
				}
...
type UnionFind struct {
	r, h, s []int // root, heigt, size
}

// Init .
func (uf *UnionFind) Init(n int) {
	uf.r = make([]int, n); for i := 0; i < n; i++ {uf.r[i] = i}
	uf.h = make([]int, n)
	uf.s = make([]int, n); for i := 0; i < n; i++ {uf.s[i] = 1}
}


// FindRoot .
func (uf *UnionFind) FindRoot(u int) int {
	if uf.r[u]==u {return u}
	uf.r[u] = uf.FindRoot(uf.r[u]); return uf.r[u]
}
