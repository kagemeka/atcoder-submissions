		...
		"ABC040": contest{
			...
			"D": func() {
				...
				for _, tmp := range q {
					...
					if y&1==1 {res[j] = uf.size[uf.FindRoot(i)]} else {uf.Unite(i, j)}
				}
...
func main() {
	...
	AtCoder.contests["ABC040"]["D"]()
	
}

// UnionFind .
type UnionFind struct {
	root, height, size []int // root, heigt, size
}

// Init .
func (uf *UnionFind) Init(n int) {
	uf.root = make([]int, n); for i := 0; i < n; i++ {uf.root[i] = i}
...
func (uf *UnionFind) FindRoot(u int) int {
	if uf.root[u]==u {return u}
	uf.root[u] = uf.FindRoot(uf.root[u]); return uf.root[u]
}

// Unite .
func (uf *UnionFind) Unite(u, v int) {
	ru, rv := uf.FindRoot(u), uf.FindRoot(v)
	if ru==rv {return}
	hu, hv := uf.height[ru], uf.height[rv]
	if hu >= hv {
		uf.root[rv] = ru 
		uf.size[ru] += uf.size[rv]
		uf.height[ru] = MaxInt(hu, hv+1)
	} else {
		uf.root[ru] = rv 
