  ...
  class ABC123:
    ...
    def d_4():
      ...
      def f(p, return_cand=False):
        ...
        for i in range(x):
          if a[i]>=p:
            cnt += y*z
            if return_cand:
              res += [a[i]+b[j]+c[k] for j in range(y) for k in range(z)]
            if cnt >= p:
              return res if return_cand else True
            continue
          for j in range(y):
            if a[i]+b[j]>=p:
              cnt += z
              if return_cand:
                res += [a[i]+b[j]+c[k] for k in range(z)]
              if cnt >= p:
                return res if return_cand else True
              continue
            for k in range(z):
              if a[i]+b[j]+c[k]<p: break
              if return_cand: res.append(a[i]+b[j]+c[k])
              cnt += 1
              if cnt == K:
                return res if return_cand else True
        return False
      ...
      def binary_search():
        ...
        res = f(p, return_cand=True)
        print(*sorted(res)[-1:-1-K:-1], sep='\n')
