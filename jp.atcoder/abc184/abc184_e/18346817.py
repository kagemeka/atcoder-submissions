...
class AtCoder:
  ...
  class ABC184:
    ...
    def c():
      ...
      if c == d == 0:
        print(0); return
      elif c+d==0 or c==d or abs(c)+abs(d)<=3:
        print(1); return
      elif abs(abs(c) - abs(d)) <= 3 or ~(c+d)&1:
        print(2); return
      else:
        print(3)
    ...
    def e():
      ...
      dic = {c: set() for c in alp}
      for i in range(h):
        for j in range(w):
          c = s[i][j]
          u = i*w+j
          if c == '#': continue
          if c in dic: dic[c].add(u)
          if c == 'S': src = u
          if c == 'G': dst = u
      ...
      while q:
        ...
        for v in g.edges[u]:
          ...
          i, j = divmod(v, w)
          c = s[i][j]
          if c in dic: dic[c].remove(v)
        i, j = divmod(u, w)
        c = s[i][j]
        if not c in dic: continue
        for v in dic[c]:
          d[v] = d[u]+1
          q.append(v)
        dic[c].clear()
