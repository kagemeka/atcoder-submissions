...
class AtCoder:
  ...
  class ABC182:
    ...
    def b():
      ...
      b = a[:,None]%i == 0
      c = b.sum(axis=0)[1:]
      c = sorted(enumerate(c), key=lambda x: (-x[1],-x[0]))
      print(c[0][0]+2)
      ...
      *n, = map(int, list(sys.stdin.readline().rstrip()))
      ...
      for i in range(1, 1<<k):
        ...
        for j in range(k):
          ...
          s += n[j]
          cnt += 1
        if s %3 == 0: res = min(res, k-cnt)
      print(-1 if res==inf else res)
      # dp = [[0]*3 for _ in range(k+1)]

      # r = sum(n) % 3
      # n = [x for x in n if x%3]
      # dp = [False]*3; dp[r] = True
      # if dp[0]: print(0); return
    ...
    def d():
      ...
      m = s.copy()
      for i in range(n-1): m[i+1] = max(m[i+1], m[i])
      s = list(itertools.accumulate(s))

      for i in range(n-1, 0, -1): s[i] = max(s[i], s[i-1]+m[i])
      s[0] = max(0, s[0])
      print(max(s))
    ...
    def e():
      ...
      r, l, u, d = g.copy(), g.copy(), g.copy(), g.copy()
      for j in range(1,w):
        bl = r[:,j] == 0
        r[bl,j] = r[bl,j-1]
