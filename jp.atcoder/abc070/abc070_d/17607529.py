...
class AtCoder:
  ...
  class ABC070:
    @staticmethod
    def d():
      n, *I = map(int, sys.stdin.read().split())
      a, b, c = np.array(I[:(n-1)*3]).reshape(-1,3).T
      d = shortest_path(csr_matrix((c, (a-1,b-1)), (n,n)), method='D', indices=I[n*3-2]-1, directed=False).astype(np.int64)
      a, b = np.array(I[n*3-1:]).reshape(-1,2).T
      print(*(d[a-1]+d[b-1]), sep='\n')
