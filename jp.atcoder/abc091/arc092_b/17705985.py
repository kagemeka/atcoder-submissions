...
class AtCoder:
  ...
  class ABC091:
    ...
    def d():
      ...
      res = 0
      for i in range(30):
        j1 = np.searchsorted(b[i], c[i]-a[i])
        j2 = np.searchsorted(b[i], 2*c[i]-a[i])
        j3 = np.searchsorted(b[i], 3*c[i]-a[i])
        j4 = np.searchsorted(b[i], 4*c[i]-a[i])
        res += ((j2-j1 + j4-j3).sum()&1) * c[i]
      print(res)
