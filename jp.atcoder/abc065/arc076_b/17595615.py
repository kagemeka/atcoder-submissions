...
class AtCoder:
  ...
  class ABC065:
    ...
    def d():
      ...
      i = np.argsort(x)
      ax, bx = i[:-1], i[1:]; cx = x[bx]-x[ax]
      i = np.argsort(y)
      ay, by = i[:-1], i[1:]; cy = y[by]-y[ay]
      a, b, c = np.hstack([ax,ay]), np.hstack([bx,by]), np.hstack([cx,cy])
      g = minimum_spanning_tree(csr_matrix((c, (a,b)), (n,n)), overwrite=True).astype(np.int64)
      print(g.sum())
