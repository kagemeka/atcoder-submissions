...
class Combinatorics:
  ...
  def make_mod_tables(self, N, n):
    ...
    if self.numpy:
      ...
      fac = [None]*(n+1); fac[0] = mint(1)

      for i in range(n): fac[i+1] = fac[i]*mint(i+1)
      ifac = [None]*(n+1); ifac[n] = fac[n]**mint(p-2)
      for i in range(n, 0, -1):
        ifac[i-1] = ifac[i]*mint(i)
      n_choose = [None]*(n+1); n_choose[0] = mint(1)
      for i in range(n): n_choose[i+1] = n_choose[i]*mint(N-i)
      for i in range(n+1): n_choose[i] = n_choose[i]*ifac[i]
    self.fac, self.ifac, self.mod_n_choose = fac, ifac, n_choose

  def mod_choose(self, n, r):
    p = self.mod
    return self.fac[n] * self.ifac[r] * self.ifac[n-r]
...
class GeometryTopology:
  class Graph:
    ...
    def dijkstra(self, src, paths_cnt=False, mod=None):
      dist = {v: inf for v in self.nodes}; dist[src] = 0
      visited = set()
      paths = {v: 0 for v in self.nodes}; paths[src] = 1
      q = [(0, src)]
      while q:
        d, u = heappop(q)
...
class AtCoder:
  ...
  class ABC021:
    ...
    def d():
      ...
      combinatorics = Combinatorics(n=2*10**5 ,numpy=False)
