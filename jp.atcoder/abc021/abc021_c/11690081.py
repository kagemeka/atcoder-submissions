...
graph = csgraph_to_dense(csr_matrix((np.ones(m), (u, v)), shape=(n, n))).astype(np.int8)
graph += graph.T

def main():
    dist = dijkstra(csgraph=graph, indices=a).astype(np.int8)
    paths = np.zeros(n, dtype=np.int64); paths[a] = 1
