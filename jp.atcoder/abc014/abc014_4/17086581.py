...
class GeometryTopology:
  class Graph:
    ...
    def get_size(self): return len(self.nodes)
    ...
    def dinic(self, src, sink):
      def bfs():
        ...
        while q:
          ...
          for v, e in self.edges[u].items():
            if e['capacity'] == 0 or v in lv: continue
            lv[v] = lv[u]+1
            q.append(v)
        return lv
      ...
      def flow_to_sink(u, flow_in):
        ...
        for v, e in self.edges[u].items():
          cap = e['capacity']
          if cap == 0 or lv[v] <= lv[u]: continue
    ...
    def init_tree(self, root=0):
      ...
      while stack:
        u = stack.pop()
        for v, e in self.edges[u].items():
          if v == self.ancestors[0][u]: continue
      ...
      for _ in range(max(self.depth).bit_length()):
        ...
        # self.ancestors.append({v: self.ancestors[-1][v] for v in self.ancestors[-1]})

    def find_dist(self, u, v):
      return self.dist[u] + self.dist[v] - 2*self.dist[self.find_lca(u, v)]
...
def cumxor(a): return reduce(op.xor, a, 0)
def cumor(a): return reduce(op.or_, a, 0)
...
class AtCoder:
  ...
  class ABC010:
    ...
    def d_3():
      ...
      for i in range(n+1): graph.add_node(i)
      for p in [int(x) for x in sys.stdin.readline().split()]:
        graph.add_edge(p, n, capacity=1)
