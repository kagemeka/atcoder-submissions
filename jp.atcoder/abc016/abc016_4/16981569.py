...
class GeometryTopology:
  class Graph:
    def __init__(self, size=10**6):
      self.size = size
      self.edges = {i:{} for i in range(size)}

    def add_edge(self, u=None, v=None, **infos): # reset edge
      self.edges[u][v] = infos

    def dinic(self, s, t): # s: source, t: sink
      def bfs():
        level = {s: 0}
        q = deque([s])
        while q:
          ...
          for v in self.edges[u]:
            if self.edges[u][v]['cap'] == 0 or v in level: continue
            level[v] = level[u]+1
            q.append(v)
        return level
      ...
      while True:
        level = bfs()
        if not t in level: return flow
        flow += flow_to_sink(s, inf)
  ...
  @staticmethod
  def triangle_area(p0, p1, p2, absolute=True):
    x1, y1, x2, y2 = p1[0]-p0[0], p1[1]-p0[1], p2[0]-p0[0], p2[1]-p0[1]
    return abs(x1*y2 - x2*y1)/2 if absolute else (x1*y2 - x2*y1)/2

  @classmethod
  def intersect(cls, seg1, seg2):
    (p1, p2), (p3, p4) = seg1, seg2
    t1 = cls.triangle_area(p1, p2, p3, absolute=False)
    t2 = cls.triangle_area(p1, p2, p4, absolute=False)
    t3 = cls.triangle_area(p3, p4, p1, absolute=False)
    t4 = cls.triangle_area(p3, p4, p2, absolute=False)
...
class AtCoder:
  ...
  class ABC012:
    ...
    def d_2():
      n, m, *abt = map(int, sys.stdin.read().split())
      graph = GeometryTopology.Graph(size=n)
