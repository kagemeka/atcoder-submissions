...
auto main() -> int {
  ...
  vector<int> dist_nodes(n);
  for(int i = 0; i < n; i++) { dist_nodes[a[i]]++; }
  if(a[0] != 0 || dist_nodes[0] != 1) {
    cout << 0 << endl;
    return 0;
  }
  ...
  vector<mint> pow_2(n);
  pow_2[0] = 1;
  for(int i = 0; i < n - 1; i++) pow_2[i + 1] = pow_2[i] * 2;
  for(int i = 1; i < n; i++) { cnt[i] = pow_2[dist_nodes[i - 1]] - 1; }
  mint p = 1;
  for(auto& d: a) { p *= cnt[d]; }
  for(auto& c: dist_nodes) { p *= power(mint(2), (long)c * (c - 1) >> 1); }
  cout << p << endl;
}
