...
class AtCoder:
  ...
  class ABC051:
    ...
    def d():
      ...
      x, y = np.array([*itertools.combinations(range(n), 2)]).T
      a, b, c = np.array(abc).reshape(m, 3).T; a -= 1; b -= 1
      d = shortest_path(csr_matrix((c, (a, b)), shape=(n, n)), method='FW', directed=False)
      print(m-np.count_nonzero((c[:,None]+d[x,a[:,None]]+d[b[:,None],y]==d[x,y]).sum(axis=1)))
