...
func NthRoot(y, n float64) (x float64) {x = y; for AbsFloat(math.Pow(x,n)-y) > Eps { x -= (math.Pow(x, n)-y)/(n*math.Pow(x, n-1))}; return}
// SortedStr .
...
// ModChoose .
func (nt *NumberTheory) ModChoose(n, r int) int {p := nt.mod; return nt.Fac[n]*nt.IFac[r]%p*nt.IFac[n-r]%p}
...
// DijkstraItem .
type DijkstraItem struct {node interface{}; dist int}
// DijkstraQueue .
...
// AStarItem .
type AStarItem struct {node interface{}; cost int; heuristicCost, score float64}
// AStarQueue .
			...
			"D": func() {
				...
				for i := 0; i < n+1; i++ {
					...
					if c[a] != -1 {
						l, d := i-c[a], c[a]
						k = ReversedString(k)
						r := make([]int, len(k)); r[0] = 1
						for i := 0; i < len(k)-1; i++ {r[i+1] = r[i]*10%l}
						m := 0
						for i := 0; i < len(k); i++ {m += r[i]*int(k[i]-'0')%l; m%=l}
						m = ((m-d)%l+l)%l
						for i := 0; i < m; i++ {a = b[a]}
...
func main() {
	// scanner.Buffer(make([]byte, 0), Inf)
	scanner.Split(bufio.ScanWords)
	AtCoder.contests["ABC030"]["D"]()
