...
class AtCoder:
  ...
  class ABC009:
    ...
    def d():
      ...
      c = [c] + [[0] * k for _ in range(k-1)]
      for i in range(k-1): c[i+1][i] = (1<<31) - 1
      ...
      def bit_matrix_pow(a, n):
        res = [[0] * len(a) for _ in range(len(a))]
        for i in range(len(a)):
          res[i][i] = (1<<31) - 1
        while n:
          if n&1: res = bit_dot(res, a)
          a = bit_dot(a, a)
          n >>= 1
        return res

      c = bit_matrix_pow(c, m-k)[0]
      res = 0
      for i in range(k): res ^= a[i] & c[i]
      print(res)
