...
class modular_int {
  ...
  template <typename U>
  static auto norm(U x) -> T {
    U m = static_cast<U>(mod());
    if (x < -m || m <= x) {
      x %= m;
    }
    return static_cast<T>(x + (x < 0) * m);
...
struct G {
  auto static op(matrix<mint> a, matrix<mint> b) -> matrix<mint> {
...
auto main() -> int {
  ...
  auto f = factorial_tables_frequent_ops<mint>(1 << 16);
  ...
  for (int k = 0; k <= d + 1; k++) {
    ...
    dp = pow_semigroup_recurse<G>(dp, n);
    cnt += dp[0][0] + dp[1][1];
  }
  cout << cnt << endl;
}
