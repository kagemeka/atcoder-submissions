...
auto main() -> int {
  ...
  // 1/2 * (m^2*\sum_i=1..n-1{i(i+1)} + n^2*\sum_i=1..m-1{i(i+1)))
  // * (n*m-2 choose k-2)
  auto f1 = [&](mint n) -> mint { return (1 + n) * n / 2; };
  auto f2 = [&](mint n) -> mint { return n * (n + 1) * (2 * n + 1) / 6; };
  mint s = 0;
  s += m * m * (f1(n - 1) + f2(n - 1));
  s += n * n * (f1(m - 1) + f2(m - 1));
  s /= 2;
  factorial_tables_frequent_ops<mint> f(1 << 18);
  s *= f.c(n * m - 2, k - 2);
  cout << s << endl;
}
