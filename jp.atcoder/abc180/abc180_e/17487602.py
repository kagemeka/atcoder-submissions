...
class AtCoder:
  ...
  class ABC180:
    ...
    def e():
      ...
      res = min(dp[-1][i] + dist[i][0] for i in range(1, n))
      print(res)
    ...
    def e_2():
      ...
      dp = np.full((1<<n, n), np.inf); dp[1][0] = 0
      dp[1][0] = 0
      for s in range(1<<n):
        i = np.arange(n)
        t = s | (1<<i)
        dp[t, i] = np.minimum(dp[t, i], (dp[s, i][:, None]+dist[:, i]).min(axis=0))

      # print((dp[-1][1:, None]+dist[1:, 0]).min().astype(int))
      print(dp[-1][0].astype(int))
