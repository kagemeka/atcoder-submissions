...
void c() {
  long long n; cin >> n;
  auto div = find_divisors(n);
  for (const auto& x : div) cout << x << '\n';
}
...
void e() {
  ...
  vector<vector<int>> dp(1<<n, vector<int>(n, numeric_limits<int>::max()/10));
  dp[1][0] = 0;
  for (int s = 0; s < (1<<n); s++) {
    for (int i = 0; i < n; i++) {
      if (s>>i&1) continue;
  ...
  int res = numeric_limits<int>::max()/10;
  for (int i = 1; i < n; i++) res = min(res, dp.back()[i]+dist[i][0]);
  cout << res << '\n';
