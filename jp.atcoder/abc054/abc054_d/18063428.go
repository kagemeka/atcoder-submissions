				...
				for i := 0; i < n; i++ {
					...
					for j := 0; j < 401; j++ {
						for k := 0; k < 401; k++ {
							dp[i+1][j][k] = dp[i][j][k]
							if j<a || k<b {continue}
							dp[i+1][j][k] = MinInt(dp[i+1][j][k], dp[i][j-a][k-b]+c)
						}
					}
				}

				res := Inf
				for i := 1; i < 400/MaxInt(ma,mb); i++ {
					res = MinInt(res, dp[n][i*ma][i*mb])
				}
