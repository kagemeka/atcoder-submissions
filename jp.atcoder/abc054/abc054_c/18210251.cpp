...
void c() {
  ...
  long long cnt = 1;
  for (auto x : f) {cnt = (cnt*(x.second+1))%MOD;}
  cout << cnt << '\n';
}
void d() {
  ...
  long long tot = 0;
  for (int i = 0; i < n-1; i++) {tot += min((x[i+1]-x[i])*a, b);}
  cout << tot << '\n';
}
}
...
void c() {
  ...
  map<P, int> cache;
  count = [&](int u=0, int s=1) {
    if (s == (1<<n)-1) return 1;
    if (cache.count(P(u,s))) return cache[P(u,s)];
    int c = 0;
    for (const auto& p : g.edges[u]) {
      int v = p.first;
      if (s>>v&1) continue;
