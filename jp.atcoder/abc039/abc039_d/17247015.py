...
class AtCoder:
  ...
  class ABC039:
    ...
    def d_2():
      ...
      black_cand = set(range(h*w)) - white
      black = set()
      for i in black_cand:
        if i%w==0: l=0
        if (i+1)%w==0: r=0
        black |= {i+dy+dx for dy in range(-w, w+1, w) for dx in range(l,r+1)}
      black &= set(range(h*w))
      for i in range(h*w):
        if s[i]=='#' and not i in black: print('impossible'); return
      print('possible')
      for i in range(h):
        print(''.join(['#' if i*w+j in black else '.' for j in range(w)]))
