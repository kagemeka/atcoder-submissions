...
func (g *Graph) FindAncestors() {
	n := g.size
	for i := 0; i < BitsLen(MaxInt(g.depth...)); i++ {
	...
	contests: map[string]contest{
		...
		"ABC019": contest{
			...
			"D": func() {
				...
				var u int
				for v := 2; v <= n; v++ {
					d := inquire(fmt.Sprintf("? %v %v", 1, v))
					if d > dist {dist = d; u = v}
				}
				for v := 1; v <= n; v++ {
					if v == u {continue}
					d := inquire(fmt.Sprintf("? %v %v", u, v))
					if d > dist {dist = d}
				}
				fmt.Printf("! %v", dist)
