...
auto main() -> int {
  ...
  sort(c.begin(), c.end());
  factorial_tables_frequent_ops<mint> f(n);
  vector<mint> b(n);
  for(int i = 0; i < n; i++) { b[i] = (i + 1) * f.c(n - 1 - i, i); }
  for(int i = 0; i < n - 1; i++) b[i + 1] += b[i];
  vector<mint> pow_2(n + 1, 1);
  for(int i = 0; i < n; i++) { pow_2[i + 1] = pow_2[i] * 2; }
  mint tot = 0;
  for(int i = 0; i < n; i++) {
    int r = n - 1 - i;
    tot += pow_2[i] * (pow_2[r] + r * pow_2[max(0, r - 1)]) * c[i];
  }
  tot *= pow_2[n];
  cout << tot << endl;
}
