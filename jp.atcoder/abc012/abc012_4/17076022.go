...
func (g *Graph) AddNode(u interface{}) {
	g.nodes[u] = true
	if g.edges[u] == nil {g.edges[u] = make(map[interface{}]*Edge)}
}
// AddEdge .
func (g *Graph) AddEdge(u, v interface{}, e *Edge) {
	if g.edges[u] == nil {g.AddNode(u)}
...
func (g *Graph) FloydWarshall() map[interface{}]map[interface{}]int {
	...
	for u := range g.nodes {
		for v := range g.nodes {d[toI[u]][toI[v]] = 1001001001001 }
	}
	for v := range g.nodes {d[toI[v]][toI[v]] = 0}
...
func (g *Graph) InitTree(root interface{}) {
	...
	// Doubling
	var maxDepth int; for _, d := range g.depth {if d > maxDepth {maxDepth = d}}

	for i := 0; i < BitsLen(maxDepth); i++ {
...
func (g *Graph) FindDist(u, v interface{}) int {
	return g.depth[u]+g.depth[v] - 2*g.depth[g.FindLCA(u, v)]
}
// FindLCA .
func (g *Graph) FindLCA(u, v interface{}) interface{} {
	...
	d := dv - du
	// up-stream 
	for i := 0; i < BitsLen(d); i++ {if d>>i & 1 == 1 {v = g.ancestors[i][v]}}
