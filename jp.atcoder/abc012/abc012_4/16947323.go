...
type Graph struct{
	...
	level []int
	shortestPath map[int]map[int]int
}
// InitLevel .
...
func (g *Graph) FloydWarshall() {
	n := g.n
	g.shortestPath = make(map[int]map[int]int)
	for i := 0; i < n; i++ {
		g.shortestPath[i] = make(map[int]int)
		for j := 0; j < n; j++ {
			g.shortestPath[i][j] = 1001001001001001
		}
	}
	for i := 0; i < n; i++ {
		g.shortestPath[i][i] = 0
	}
	for i := 0; i < n; i++ {
		if g.edges[i] == nil {continue}
		for j := 0; j < n; j++ {
			if g.edges[i][j] == nil {continue}
			g.shortestPath[i][j] = g.edges[i][j].weight 
		}
	}
	for k := 0; k < n; k++ {
		for i := 0; i < n; i++ {
			for j := 0; j < n; j++ {
				g.shortestPath[i][j] = MinInt(
					g.shortestPath[i][j],
					g.shortestPath[i][k] + g.shortestPath[k][j],
				)
			}
		}
	}
	...
	contests: map[string]contest{
		...
		"ABC012": contest{
			...
			"D": func() {
				...
				for i := 0; i < n; i++ {
					for j := 0; j < n; j++ {
						maxDist[i] = MaxInt(maxDist[i], g.shortestPath[i][j])
					}
				}
