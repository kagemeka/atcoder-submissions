...
auto main() -> int {
  ...
  vector<int> edge_cnt(1 << n);
  for(int s = 1; s < 1 << n; s++) {
    int i = __builtin_ctz(s);
    edge_cnt[s] = edge_cnt[s & (s - 1)] + __builtin_popcount(s & g[i]);
  }
  vector<mint> pow2(m + 1, 2);
  pow2[0] = 1;
  for(int i = 0; i < m; i++) pow2[i + 1] *= pow2[i];
  vector<mint> dp(1 << n);
  for(int s = 0; s < 1 << n; s++) {
    if(~s & 1) continue;
    dp[s] = pow2[edge_cnt[s]];
    for(int t = s; t;) {
      t = (t - 1) & s;
      if(~t & 1) continue;
      dp[s] -= dp[t] * pow2[edge_cnt[s ^ t]];
    }
  }
  vector<mint> res(n);
  for(int s = 0; s < 1 << n; s++) {
    for(int i = 0; i < n; i++) {
      if(~s >> i & 1) continue;
      res[i] += dp[s] * pow2[edge_cnt[((1 << n) - 1) ^ s]];
    }
  }
