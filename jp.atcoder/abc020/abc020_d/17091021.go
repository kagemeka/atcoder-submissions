...
// Init (NumberTheory) .
func (nt *NumberTheory) Init(n int) {
...
func samplePrimeFactorize() {
	nt := new(NumberTheory); nt.Init(2e6)
...
// 	// tree
// 	depth map[interface{}]int 
// 	dist map[interface{}]int
// 	ancestors []map[interface{}]interface{}
// }
// // Init .
// func (g *Graph) Init() {
// 	g.nodes = make(map[interface{}]bool)
...
// 	// if _, ok := g.edges[u]; !ok {g.AddNode(u)}
// 	g.AddNode(u)
// 	g.AddNode(v)
// 	if _, ok := g.edges[u]; !ok {g.edges[u] = make(map[interface{}]*Edge)}

// 	// if !g.nodes[v] {g.AddNode(v)}
// 	g.edges[u][v] = e 
// }
...
// 	toI := make(map[interface{}]int) // toIndex
// 	toN := make([]interface{}, n) // toNode
// 	i := 0
// 	for v := range g.nodes {
// 		toI[v] = i 
// 		toN[i] = v
...
// 	for u := range g.nodes {
// 		for v := range g.nodes {d[toI[u]][toI[v]] = 1001001001001001 }
// 	}
// 	for i := 0; i < n; i++ {d[i][i] = 0}
// 	for u, tmp := range g.edges {
// 		for v, e := range tmp {d[toI[u]][toI[v]] = e.weight}
// 	}
// 	for k := 0; k < n; k++ {
// 		for i := 0; i < n; i++ {
// 			for j := 0; j < n; j++ {
...
// 	for i := 0; i < n; i++ {
// 		d2[toN[i]] = make(map[interface{}]int)
// 		for j := 0; j < n; j++ {
// 			d2[toN[i]][toN[j]] = d[i][j]
// 		}
// 	}
// 	return d2
// }

// // func (g *graph) Dijkstra(u interface{}) map[interface{}]int {
// // }
...
// 		flow := 0 
// 		for v, e := range g.edges[u] {
// 			if e.cap==0 || lv[v]<=lv[u] {continue}
// 			f := flowToSink(v, MinInt(flowIn, e.cap))
// 			if f == 0 {continue}
// 			g.edges[u][v].cap -= f; g.edges[v][u].cap += f
// 			flowIn -= f; flow += f 
// 		}
// 		return flow
// 	}
...
// 	// n := g.GetSize()
// 	g.depth = make(map[interface{}]int)
// 	g.dist = make(map[interface{}]int)
// 	g.ancestors = make([]map[interface{}]interface{}, 1)
// 	g.ancestors[0] = make(map[interface{}]interface{})
...
// 	// Doubling
// 	maxDepth := 0; for _, d := range g.depth {if d > maxDepth {maxDepth = d}}

// 	for i, l := 0, BitsLen(maxDepth); i < l; i++ {
// 		ancestor := g.ancestors[len(g.ancestors)-1]
// 		nxtAncestor := make(map[interface{}]interface{})
// 		for v := range g.nodes {
// 			nxtAncestor[v] = ancestor[ancestor[v]]
// 		}
	...
	contests: map[string]contest{
		...
		"ABC020": contest{
			...
			"D": func() {
				...
				for i := l-1; i > -1; i-- {
					for j := i+1; j < l; j++ {
						if div[j]%div[i] == 0 {s[i] = (s[i]-s[j])%Mod}
					}
				}
				tot := 0
				for i := 0; i < l; i++ {
...
func main() {
	...
	AtCoder.contests["ABC020"]["D"]()
	// AtCoder.contests["ABC014"]["D"]()
