import sys
import numpy as np
from scipy.sparse.csgraph import shortest_path, maximum_flow, minimum_spanning_tree
...
class NumberTheory:
  ...
  def sieve_of_eratosthenes(self, n):
    if self.np_flg:
      sieve = np.ones(n+1, dtype=np.int32); sieve[:2] = 0
...
class GeometryTopology:
  class Graph:
    ...
    def add_vertex(self, v, **info):
      self.vertices[v] = info
      self.size = len(self.vertices)
      if not v in self.edges: self.edges[v] = {}

    def add_edge(self, u, v, **info): # reset edge
      if not u in self.edges: self.edges[u] = {}
      if not u in self.vertices: self.add_vertex(u)
    ...
    def dinic(self, source, sink):
      def bfs():
        ...
        while q:
          u = q.popleft()
          for v, info in self.edges[u].items():
            if info['cap'] == 0 or v in level: continue
            level[v] = level[u]+1
            q.append(v)
        return level
      ...
      def flow_to_sink(u, flow_in):
        ...
        for v, info in self.edges[u].items():
          cap = info['cap']
          if cap == 0 or level[v] <= level[u]: continue
          f = flow_to_sink(v, min(flow_in, cap))
          if not f: continue
          self.edges[u][v]['cap'] -= f
          self.edges[v][u]['cap'] += f
          flow_in -= f
          flow += f
        return flow
      ...
      while True:
        level = bfs()
        if not sink in level: return flow
        flow += flow_to_sink(source, inf)
    ...
    def floyd_warshall(self):
      v = self.vertices
      d = {u: {v: inf for v in self.vertices} for u in self.vertices}
      # d = [[inf] * n for _ in range(n)]
      for v in self.vertices: d[v][v] = 0
    ...
    def dijkstra(self, source):
      ...
      while q:
        ...
        for v, info in self.edges[u].items():
          ...
          heappush(q, (d+info['weight'], v))
      return dist

    def a_star(self, source, target, heuristic_func):
      dist = {v: inf for v in self.vertices}
      q = [(heuristic_func(source, target), 0, source)]
      while q:
        s, c, u = heappop(q)
  ...
  class TreeGraph:
    def __init__(self, size=None, edges=[], root=0):
      self.size = size if size is not None else len(edges)
      self.edges = edges
      self.init_root(root)
      self.find_ancestors() # 2^0, 2^1, 2^2, ... 2^k, ...
    ...
    def find_ancestors(self):
      ...
      for _ in range(max(self.depth).bit_length()):
        ancestors.append([ancestors[-1][a] for a in ancestors[-1]])
    ...
    def upstream(self, v, d):
      for i in range(d.bit_length()):
        if d>>i & 1: v = self.ancestors[i][v]
      return v
...
class AtCoder:
  ...
  class ABC002:
    ...
    def d():
      ...
      print(max(len(s) for i in range(1, 1<<n) for s in [[j for j in range(n) if i>>j&1]] if all((x, y) in edges for x, y in combinations(s, 2))))
  ...
  class ABC010:
    ...
    def d_3():
      ...
      for i in range(n+1): graph.add_vertex(i)
      for p in [int(x) for x in sys.stdin.readline().split()]:
        graph.add_edge(p, n, cap=1)
        graph.add_edge(n, p, cap=0)
      for a, b in zip(*[map(int, sys.stdin.read().split())]*2):
        graph.add_edge(a, b, cap=1)
        graph.add_edge(b, a, cap=1)
  ...
  class ABC018:
    ...
    def d():
      ...
      g = np.array([*combinations(range(n), p)])
  ...
  class ABC020:
    ...
    def c():
      ...
      def min_time(x):
        ...
        # return graph.a_star(source, target, heuristic_function)
