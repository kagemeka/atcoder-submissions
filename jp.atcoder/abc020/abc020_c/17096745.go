...
func (g *Graph) Dijkstra(src  interface{}) map[interface{}]int {
	dist := make(map[interface{}]int)
	for v := range g.nodes {dist[v] = math.MaxInt64}
	...
	for len(*h) > 0 {
		...
		if dist[u] != math.MaxInt64 {continue}
		dist[u] = d
		for v, e := range g.edges[u] {
			if dist[v] != math.MaxInt64 {continue}
			heap.Push(h, &DijkstraItem{node: v, dist: d+e.weight})
		}
	}
	return dist 
}
		...
		"ABC020": contest{
			...
			"C": func() {
				...
				heuristicFunc := func(u, tgt interface{}) float64 {
					...
					b := tgt.(PairInt)
					vx, vy := b.x, b.y
					return math.Abs(float64(vx-ux)) + math.Abs(float64(vy-uy))
				}
