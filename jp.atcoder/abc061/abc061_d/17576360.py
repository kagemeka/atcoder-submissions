...
class AtCoder:
  ...
  class ABC061:
    ...
    def d():
      n, m, *abc = map(int, sys.stdin.read().split())
      abc = list((a-1, b-1, -c) for a, b, c in zip(*[iter(abc)]*3))
      d = [inf] * n; d[0] = 0
      for i in range(n-1):
        for a, b, c in abc: d[b] = min(d[b], d[a]+c)
      neg_cycle = [False] * n
      for a, b, c in abc:
        if d[a]+c < d[b]: neg_cycle[b] = True
      for a, b, c in abc: neg_cycle[b] += neg_cycle[a]
      print(inf if neg_cycle[-1] else -d[-1])
    ...
    def d_2():
      ...
      a -= 1; b -= 1; c *= -1
      g = csr_matrix((c, (a, b)), (n, n))
      gg = csr_matrix(([1]*(m+1), (np.append(a, n-1), np.append(b, 0))), (n, n))
      _, labels = connected_components(gg, connection='strong')
      i = labels==labels[0]
      g = g[i,:][:,i]
    ...
    def d_3():
      ...
      abc = list((a-1, b-1, -c) for a, b, c in zip(*[iter(abc)]*3))
      g = GeometryTopology.Graph(n)
      for a, b, c in abc: g.add_edge(a, b)
      g.add_edge(n-1, 0)
      r = g.scc()
      g = GeometryTopology.Graph(n)
      for a, b, c in abc:
        if r[a] != r[0] or r[b] != r[0]: continue
        g.add_edge(a, b, weight=c)
      try: print(-g.bellman_ford(0)[-1])
      except: print('inf')
