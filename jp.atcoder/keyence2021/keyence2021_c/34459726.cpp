...
auto main() -> int {
  ...
  vector<mint> dp(w);
  dp[0] = 1;
  vector<mint> pow3(w + 1);
  pow3[0] = 1;
  for(int i = 0; i < w; i++) pow3[i + 1] = pow3[i] * 3;
  vector<int> f_row(h, w), f_col(w, h);
  ...
  for(int i = 0; i < h; i++) {
    ...
    for(int j = 0; j < w; j++) {
      ...
      if(t[j] != 'D' && j < w - 1) { dp[j + 1] += dp[j] * pow3[f_col[j]]; }
      if(t[j] == '-' && j < w - 1) { dp[j + 1] += dp[j] * pow3[f_col[j]]; }
      if(t[j] != 'R') { ndp[j] += dp[j] * pow3[f_row[i]]; }
      if(t[j] == '-') { ndp[j] += dp[j] * pow3[f_row[i]]; }
    }
    if(i < h - 1) swap(dp, ndp);
  }
  mint res = dp[w - 1];
  if(s[h - 1][w - 1] == '-') res *= 3;
  cout << res << endl;
